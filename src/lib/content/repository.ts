...
function paginate(total: number, query: ContentListQuery) {
...
function mapBlog(row: any, lang: Lang): StudentNewsPost {
  const map = resolveLocale(row);
  ...
  return {
    id: row.id,
    locale: lang,
    slug: row.slug || slugify(loc.title),
    dateIso,
    date: formatDate(dateIso),
    category: canonicalCategory(map),
    title: loc.title,
    excerpt: stripHtml(loc.description || "").slice(0, 180),
...
function mapActivity(row: any, lang: Lang): ActivityNewsItem {
  const map = resolveLocale(row);
...
function mapPhoto(row: any): PhotoGalleryItem {
  const map = resolveLocale(row);

  const loc = pickLocalizedFields(map, "en");
...
function mapVideo(row: any): VideoGalleryItem {
  const map = resolveLocale(row);
  ...
  return {
    ...
    category: canonicalCategory(map),
    title: loc.title,
    thumbClass: "from-blue-500 to-sky-400",
...
export async function listBlogPosts(
  ...
  try {
    ...
    const total = isHome
      ? HOME_LIMIT_FALLBACK
      : await dashboardPrisma.dashboardBlog.count({
          where,
        });
    ...
    const items = STUDENT_NEWS_POSTS.map((post) => ({
      ...post,
      locale: requestLang,
    }));
...
export async function listActivityItems(
  ...
  try {
    ...
    const items = ACTIVITY_NEWS.map((item) => ({
      ...item,
      locale: requestLang,
    }));
...
export async function getActivityItem(
  ...
  try {
    ...
    const staticMatch = getActivityBySlug(slug);
    ...
    console.error("[getActivityItem]", error);
...
export async function listPhotoItems(
  ...
  try {
    const rows = await dashboardPrisma.dashboardPhoto.findMany({
      orderBy: {
        createdAt: "desc",
      },
    });
    ...
    const total = PHOTO_GALLERY_ITEMS.length;
...
export async function listVideoItems(
  ...
  try {
    ...
    const total = VIDEO_GALLERY_ITEMS.length;
