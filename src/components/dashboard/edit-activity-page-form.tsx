...
export function EditActivityPageForm({ id }: { id: string }) {
    ...
    const load = async () => {
      ...
      try {
        const [itemRes, categoryRes] = await Promise.all([
          fetch(`/api/dashboard/activity/${id}`, { cache: "no-store" }),
          fetch("/api/dashboard/categories?type=activity", { cache: "no-store" }),
        ]);
        if (!itemRes.ok) {
        ...
        const item = (await itemRes.json()) as ActivityRow;
        ...
        if (!cancelled) notify("Could not load activity.", "error");
      } finally {
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Edit Activity</h1>
        ...
        {loading || !localeContent || !block ? (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-activity-title">Title ({editLocale})</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-activity-category">Category ({editLocale})</Label>
              <select
