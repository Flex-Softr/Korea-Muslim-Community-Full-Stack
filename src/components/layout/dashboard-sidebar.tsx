...
export function DashboardSidebar({ role }: { role: UserRole }) {
  ...
  return (
    <aside
      className={cn(
        "hidden h-full min-h-0 shrink-0 flex-col border-e border-emerald-800/60 bg-linear-to-b from-emerald-900 via-emerald-900 to-emerald-800 text-white transition-all duration-200 lg:flex lg:shadow-xl lg:backdrop-blur-sm",
        collapsed ? "w-18" : "w-[16rem] xl:w-68",
      )}
    >
      <div
        className={cn(
        ...
        <span className="inline-flex items-center gap-3 rounded-xl bg-white/6 px-3 py-2">
          <span className="inline-flex size-8 items-center justify-center rounded-lg bg-white/10 text-emerald-100">
            <span className="grid size-4 grid-cols-2 gap-0.5">
              <span className="rounded-xs bg-current" />
              <span className="rounded-xs bg-current" />
              <span className="rounded-xs bg-current" />
              <span className="rounded-xs bg-current" />
            </span>
      ...
      <div
        className={cn(
          "space-y-2 border-t border-white/10 py-3",
          collapsed ? "px-2" : "px-3",
        )}
      >
        <button
