...
export async function PATCH(
  ...
  const existing = await getDashboardActivityById(id);
  if (!existing) {
  ...
  const updated = await updateDashboardActivity(id, {
...
export async function GET(
  ...
  const row = await getDashboardActivityById(id);
  if (!row) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
  const canManageAll = hasMinimumRole(session.user.role, "ADMIN");
...
export async function DELETE(
  ...
  const removed = await deleteDashboardActivity(id);
  if (!removed) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
