...
export async function GET(request: Request) {
  ...
  if (mine) {
    ...
    return NextResponse.json({ items: await listDashboardBlogsByCreator(session.user.id) });
  }
  return NextResponse.json({
    items: await listDashboardBlogs(),
...
export async function POST(request: Request) {
  ...
  const created = await createDashboardBlog({
    sourceLocale,
    title: body.title,
    ...
    status:
      hasMinimumRole(session?.user?.role, "ADMIN") ? "published" : undefined,
  });
  revalidateCmsContent("blog");
