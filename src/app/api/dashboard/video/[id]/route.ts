...
export async function PATCH(
  ...
  const existing = await getDashboardVideoById(id);
  if (!existing) {
  ...
  const updated = await updateDashboardVideo(id, {
...
export async function GET(
  ...
  const row = await getDashboardVideoById(id);
  if (!row) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
  const canManageAll = hasMinimumRole(session.user.role, "ADMIN");
...
export async function DELETE(
  ...
  const removed = await deleteDashboardVideo(id);
  if (!removed) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
  revalidateCmsContent("video");
